...
Version 0.13.0 adds a privacy-preserving security audit ledger. `/audit`, `/audit approvals`, and `/audit credentials` show recent local approval and credential-lifecycle facts without storing command text, prompts, file contents, full workspace paths, or credentials. Local 0.13.1 development now includes versioned task-run journals, explicit interrupted-task recovery, and programmatic blocking of automatic side-effect replay; it remains unpublished until forced-termination and release-gate validation completes.
