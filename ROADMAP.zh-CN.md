...
| npm 包 | `@xiu-ai/cli` |
| 全局命令 | `xiu` |
| 当前公开版本 | `0.16.5`，运行过程持久可见 |
| 当前开发版本 | `0.17.0`，可验证的产品基础 |
...
1. 完成 v0.17.0 CI、文档和本地打包冒烟验证。
2. 观察三平台 CI 结果并形成真实失败清单，不提前修改平台支持声明。
3. 完成首次运行外部验收后，由用户决定是否提交、推送与发布 v0.17.0。
