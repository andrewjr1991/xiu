...
| 当前开发版本 | `0.18.0`，真实 v0.17.0 基线续跑闭环实施中 |
...
1. 完成真实评测的 trial 边界续跑、来源摘要绑定、累计预算和不可重放门禁；普通 PR CI 仍不获得凭证或运行真实模型。
2. 已有两次真实基线尝试因单任务模型调用预算安全停止，分别记录 2 个和 3 个 trial，累计 38 次模型调用、51 次工具调用，估算费用均为 0 USD；这些部分报告不作为正式基线。
3. 使用带续跑元数据的当前任务修订重新运行精确 v0.17.0 发布工件，完成每题三次基线并人工复核，再决定是否开展优化和扩充至 20 个任务。
