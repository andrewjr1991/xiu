...
| 当前开发版本 | `0.18.0`，可重复评测与成本基线（设计评审中） |
...
1. 审核 `V0.18.0_DESIGN.zh-CN.md` 中的任务分布、评测协议、安全隔离和发布门槛。
2. 方案获批后实现评测协议、运行器、报告器和首批 10 个任务；普通 PR 只接入确定性冒烟。
3. 使用精确 v0.17.0 发布工件完成真实模型基线并人工复核，再决定是否开展优化和扩充至 20 个任务。
