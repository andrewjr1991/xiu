...
运行期输入框下方默认常驻紧凑任务进度，包括当前 Turn、阶段、耗时、待注入补充、显式队列、步骤完成情况、`Now` 当前动作、`Next` 下一步以及最近成功的文件写入或修改。
...
从 v0.8.2 开始，模型在重要阶段切换时会给出一句简短进展说明，显示为 `Update:`。它应说明刚确认了什么、当前正在做什么以及下一步是什么，不会逐条复述每个普通工具调用。

从 v0.8.3 开始，成功的文件写入会作为永久终端记录显示，不再只停留在会被刷新的底部状态栏。每张 `File changes` 卡片会标明 `Created`、`Modified` 或 `Deleted`、文件路径、`+新增行/-删除行`，并显示最多 6 行经过宽度限制的关键增删内容。超过 1 MB 的文本及二进制文件只显示修改前后的字节规模，避免大文件内容刷屏。修改发生时 Xiu 会安全重绘运行期输入框；任务结束前还会补打印未展示的卡片，因此不会因终端刷新或结束竞态丢失。

`Update:` 回答“当前阶段在做什么”，`File changes` 回答“工作区实际改了什么”，`Ctrl+O` 回答“具体调用过哪些工具”。三者用途不同，重要修改记录不会再依赖模型是否主动总结。
...
首次由 Xiu 主动读取 Windows 剪贴板时，Xiu 会使用 Windows 自带的 .NET Framework 编译器，在 `~/.xiu/cache/clipboard-helper-v1/` 生成固定的本地剪贴板读取器。它只读取剪贴板、保存 PNG 或返回复制的文件路径，不上传附件。若系统缺少该编译器，Xiu 会显示明确错误，终端原生文字粘贴和 `@path` 仍可使用。
