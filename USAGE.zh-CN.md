...
从 v0.7.1 开始，Agent 工作时输入框不会消失，而是变为：

```text
xiu[working]>
```

此时可以继续输入下一项要求并按 Enter。Xiu 会按提交顺序排队，当前任务结束后在同一会话中依次处理。状态栏会显示当前阶段和排队数量。运行期输入仍支持多行、中文、Emoji、`@` 文件补全和草稿恢复。
...
| 普通文本 + Enter | 加入后续任务队列 |
| `/queue` | 查看尚未开始的后续要求 |
| `/clear-queue` | 清空尚未开始的要求 |
| `/cancel` 或 `Ctrl+C` | 取消当前任务，保留队列 |
| `/status` | 查看当前阶段和调用统计 |
| `/exit` | 取消当前任务、清空队列并退出 |

需要写入、执行或危险操作审批时，Xiu 会先挂起运行期输入框，再显示审批菜单。审批默认仍是“拒绝”；审批结束后，未提交的草稿会恢复。

v0.7.1 的队列只保存在当前 Xiu 进程中。强制关闭终端后，已经开始的会话可以恢复，未提交草稿也会恢复，但尚未开始的排队项不会自动恢复。
...
模型思考或工具执行过程中按 Ctrl+C，会取消当前任务，但保留已经提交的后续队列。需要清空队列可输入 `/clear-queue`；需要取消、清空并退出可输入：
