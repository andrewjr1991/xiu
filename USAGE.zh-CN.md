...
托管搜索的设备登记、短期 Token 签发、`web_search` 和 `web_open` 使用同一套独立联网代理，不再继承当前模型 Provider 的代理。默认直连；只有搜索服务确实需要代理时，才在 `/web configure` 中单独填写，或为内置托管搜索设置 `XIU_WEB_PROXY`。这样 Agnes 可以走代理，而无需代理的其他模型和联网搜索仍可直连。
...
- 使用独立的联网搜索代理；默认直连，可通过 `/web configure` 或 `XIU_WEB_PROXY` 显式设置。
