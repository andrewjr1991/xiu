...
为避免反复换关键词和不可访问页面导致长时间空转，同一任务最多执行 3 次真实 `web_search` 搜索发现，每个 `web_open` 页面只尝试一次。尚无成功来源时最多允许 6 次真实页面打开失败；已有至少一个来源成功打开后，上限收紧为 4 次。达到上限后，Xiu 不再执行任何联网或其他工具，只给模型一次“仅作答”机会，并明确列出本次唯一允许引用的精确 URL；最终答复只能使用这些已经成功打开并通过精确 URL 来源门禁的来源，数量不足必须少答。若模型在混合结果中仍夹带未打开链接，Xiu 会删除该链接所在的完整候选条目，而不是只删链接留下无来源陈述；只要剩余内容仍包含已打开来源，并为每个引用条目保留真实日期或确定性“日期未知”标记，任务会以较少条目正常完成。无法安全保留核验结果、模型继续请求工具或剩余证据仍不足时，任务立即以本地化错误结束，不会再进入纠正循环。
