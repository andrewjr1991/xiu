...
右键粘贴由终端而不是 Xiu 处理：普通文字和资源管理器复制的文件具有字符或路径表示，因此通常可以右击粘贴；剪贴板中的纯位图没有可发送给 CLI stdin 的字符，Windows Terminal 右击时不会通知 Xiu。此类图片请使用 `Ctrl+V` 或 `/paste`。Xiu 不默认接管全局鼠标事件，因为这会破坏终端文本选择、右键菜单和滚轮行为。
