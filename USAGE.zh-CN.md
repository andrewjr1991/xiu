...
从 v0.8.1 开始，除标准的 test、typecheck、lint 和 build 命令外，Xiu 也会识别项目自定义的验证脚本，例如 `verify_output.py`、`check-result.js`、`output_validate.py`。只有命令返回 `Exit code: 0` 才算验证通过；脚本名包含验证含义但执行失败时，任务仍会标记为未验证。

v0.8.2 进一步识别包含“验证”或“校验”的多行 `python -c` 等内联检查。任务结束时，运行期间隐藏的工具日志不会再全部打印到终端；默认只显示模型最终回答和 `Done/Stopped` 摘要。完整工具证据仍保存在活动详情和会话日志中。
